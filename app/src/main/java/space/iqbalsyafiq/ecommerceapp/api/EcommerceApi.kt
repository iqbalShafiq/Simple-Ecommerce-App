...
interface EcommerceApi {
    // register user
    @POST("test_ecommerce.apis.v1.user.add")
    fun registerUser(
    ...
    // login user
    @POST("test_ecommerce.apis.v1.user.auth")
    fun loginUser(
        @Header("Authorization") token: String,
    ...
    // tokenize
    @POST("test_ecommerce.apis.v1.user.tokenize")
    fun tokenize(
        @Header("Authorization") token: String,
    ...
    // check session
    @POST("test_ecommerce.apis.v1.user.check_session")
    fun checkSession(
        @Header("Authorization") token: String
    ): Call<MessageResponse>

    // get list item
    @POST("test_ecommerce.apis.v1.item.get")
    fun getListItem(
        @Header("Authorization") token: String,
    ...
    // add cart
    @POST("test_ecommerce.apis.v1.cart.add")
    fun addChart(
        @Header("Authorization") token: String,
    ...
    // get cart
    @POST("test_ecommerce.apis.v1.cart.get")
    fun getCart(
        @Header("Authorization") token: String
    ): Call<CartResponse>
}